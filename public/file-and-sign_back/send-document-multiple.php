...
if(isset($_POST['submit']))
{
	...
	if(is_array($to_email_arr)) {
		...
		$send_arr['next_id'] = 1;
		$r_id = ins_rec('document_send',$send_arr);
	}
	
			$doc_link = $SITE_URL."document-sign-multiple.php?data=".$session_id."&&difs=".$d_id."&Oid=".$owner_id."&Nid=1&r_id=".$r_id."&Signer=".$to_email_arr[0];
			$subject= $subject;
			$to=$to_email_arr[0];
				...
				<div class="form-group">
					<label style="float:left; width:100%!important">Email to 
						<div style="float:right">
							...
							<input type="checkbox" checked="checked" style="height:20px; width:100%; margin:0;" onclick="window.location.href='<?=$SITE_URL?>send-document.php?data=<?=$session_id?>&difs=<?=$d_id?>&Oid=<?=$owner_id?>&doc_name=<?=$doc_name?>'" />
							</div>
					...
					<div id="email_<?=$i?>" <?=($i==0)?"style='display:block'":"style='display:none'";?>>
						...
						<input type="text" <?php if($i==0) {?> id="email1" <?php } ?> name="to_email[]" class="form-control" placeholder="example<?=$i+1?>@email.com">
						<?php if($i<4) { ?>
