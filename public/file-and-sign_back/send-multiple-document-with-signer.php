...
if(isset($_POST['submit']))
{
	extract($_POST);
		...
		$documentArr = array_filter($documents);
		$document_str = implode(',',$documentArr);
		if(is_array($to_email_arr)) {
			$email_arr = implode(',',array_filter($to_email_arr));
			$send_arr['email_arr'] = $email_arr;
			$send_arr['email_subject'] = $subject;
			$send_arr['email_body'] = $message;
			$send_arr['doc_id'] = $d_id;
			$send_arr['next_id'] = 1;
			$send_arr['send_type'] = 1;
			$send_arr['docs_id'] = $document_str;
			$r_id = ins_rec('document_send_multiple',$send_arr);
		}
		$Signer = $to_email_arr[0];
		...
		if(count($documentArr) > 0) {
			...
			$doc_link = $SITE_URL."document-sign-multiple-with-signer.php?data=".$session_id."&&difs=".$doc_id."&Oid=".$owner_id."&Nid=1&r_id=".$r_id."&Signer=".$Signer;	
			$mailcontent .=	'
				<tr>
					<td align="center">
		...
		$mailcontent .=	'
			...
			//echo $to."<br>".$subject."<br>".$mailcontent."<br>".$from; die(); 
...
<body>
	<div class="row">
		...
		<div class="alert alert-info" style="margin:0;">
			<strong>SEND MULTIPLE DOCUMENT WITH MULTI SIGNER(CHAIN SIGN)</strong>
		</div>
				...
				<div class="form-group">
					<label style="float:left; width:100%!important"> 
						<div style="float:right">
							...
							<input type="checkbox" id="checkbox1" checked="checked" style="height:20px; width:100%; margin:0;" onclick="window.location.href='<?=$SITE_URL?>send-multiple-document.php?data=<?=$session_id?>&difs=<?=$d_id?>&Oid=<?=$owner_id?>&doc_name=<?=$doc_name?>'" />
							</div>
