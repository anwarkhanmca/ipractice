...
$('.addnew_line').click(function() {
    ...
    $newstaffRow.find('input[type="hidden"]').attr('id', 'notesstaff'+ noOfDivs);
    $newstaffRow.find(":button").attr('data-key', noOfDivs);
    
    $('#sBoxTable tr:last').after($newstaffRow);
    $('#notesstaff'+noOfDivs).val('');
    $(".dpick").datepicker({dateFormat: 'dd-mm-yy'}); 
    ...
    function staffnotes(){
        ...
        var key             = $("#hidNoteKey").val();

        $('#notesstaff'+key).val(staffnotesval);
        $("#addfontnotes-modal").modal("hide");
...
<input type="hidden" id="encode_start_date" value="{{ base64_encode($start_date) }}" />
 <div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <td width="6%"><input type="text" id="head_days_taken" style="width:50px;text-align: center; border-radius: 5px" disabled/></td>
    <td width="20%"><div id="remainDiv">Days Remaining <input type="text" id="head_days_remain" style="width:50px;text-align: center; border-radius: 5px" disabled></div></td>
    <!-- <td width="13%"></td> -->
  </tr>
</table>
...
@if($staff_type == "staff")
    <thead>
      <tr role="row">
        <th align="left" width="3%"><input type="checkbox" id="allCheckSelect"/></th>
        <th align="center" width="20%">Staff Name</th>
    ...
    @if(!empty($awating_staff))
	    @foreach($awating_staff as $key=>$staff)
            <tr>
                ...
                <th align="center"><a href="javascript:void(0)" class="editrequest" data-rowid="{{$staff['holidayrequest_id']}}">{{$staff['staff_detail']['fname'] or ""}} {{$staff['staff_detail']['lname'] or ""}}</a></th>
                <td align="left">{{$staff['type_name'] or ""}}</td>
                <td align="center">
                    <div id="edit_calender_{{ $staff['holidayrequest_id'] }}_21" class="edit_cal">
                    <a href="javascript:void(0)" id="date_view_{{ $staff['holidayrequest_id'] }}_21" />{{ $staff['date'] }}</a>
                    <span class="glyphicon glyphicon-chevron-down open_adddrop" data-client_id="{{ $staff['holidayrequest_id'] or "" }}" data-tab="21"></span>
                    <div class="cont_add_to_date open_dropdown_{{ $staff['holidayrequest_id'] }}_21" style="display:none;">
                    <ul> 
                      <!-- <li><a href="javascript:void(0)" class="open_calender_pop" data-client_id="{{ $staff['holidayrequest_id'] or "" }}" data-tab="21">Add/Edit Start Date</a></li>
                      <li> -->
                      <span id="view_calender_{{ $staff['holidayrequest_id'] }}_21" class="addtocalendar atc-style-blue">
                        <var class="atc_event">
                          <var class="atc_date_start">{{ (isset($staff['date']) && $staff['date'] != "")?date("d-m-Y H:i", strtotime($staff['date']) ):"" }}</var>
                          <var class="atc_date_end">{{ (isset($staff['date']) && $staff['date'] != "")?date("Y-m-d H:i:s", strtotime('+4 hour', strtotime($staff['date'])) ):"" }}</var>
                          <var class="atc_timezone">Europe/London</var>
                          <var class="atc_title">{{$title}} - {{ $staff['type_name'] or ""}}</var>
                          <var class="atc_description">{{$title}} - {{ $staff['type_name'] or ""}}</var>
                ...
                <td align="center">
                @if(isset($staff['state']) && $staff['state'] == 'Approved')
                    <input type="button" value="{{$staff['state'] or ""}}" class="approved_btn">
                @elseif(isset($staff['state']) && $staff['state'] == 'Declined')
                ...
                @if(empty($staff['notes']))
                    <a href="javascript:void(0)" onclick='return fetchnotesmodal("{{ $staff['holidayrequest_id'] }}")' data-toggle="modal" data-target="#fetchcomposenotes-modal"><span class="btn btn-default note_t">Notes</span>
                    </a>
                @endif
                @if(!empty($staff['notes']))
                    <a href="javascript:void(0)" onclick='return fetchnotesmodal("{{ $staff['holidayrequest_id'] }}")' data-toggle="modal" data-target="#fetchcomposenotes-modal"><span style="border-bottom:3px dotted #3a8cc1 !important" class="btn btn-default note_t">Notes</span>
                    </a>
                @endif
                </td>
        ...
        <td align="center">
            <select class="form-control statechage" data-date="{{ $staff['date'] or ""}}" data-staff_name="{{$staff['staff_detail']['fname'] or ''}} {{$staff['staff_detail']['lname'] or ''}}" data-format_date="{{ date('l, F d, Y', strtotime($staff['date']))}}" data-staff_id="{{ $staff['staff_detail']['user_id'] }}">
            ...
            @elseif(isset($staff['state']) && $staff['state'] == 'Declined')
                ...
                <option value="archive||{{$staff['holidayrequest_id']}}" >Archive</option>
            @else
                <option value="Who else is off||{{$staff['holidayrequest_id']}}" {{ (isset($staff['state']) && $staff['state'] == "Who else is off")?"selected":"" }}>Who else is off</option>
                <option value="Check Tasks Deadlines||{{$staff['holidayrequest_id']}}" {{ (isset($staff['state']) && $staff['state'] == "Check Tasks Deadlines")?"selected":"" }}>Check outstanding Jobs</option>
                <option value="Approved||{{$staff['holidayrequest_id']}}" {{ (isset($staff['state']) && $staff['state'] == "Approved")?"selected":"" }}>Approve</option>
                <option value="Declined||{{$staff['holidayrequest_id']}}" {{ (isset($staff['state']) && $staff['state'] =="Declined")?"selected":"" }}>Decline</option>
    ...
    <tbody role="alert" aria-live="polite" aria-relevant="all">
     ...
     @if(!empty($profilesection))
		@foreach($profilesection as $key=>$prof)
            @if($prof->state == "Awaiting Approval" || $prof->state == "Request Withdrawn" || $prof->state == "Approved" || $prof->state == "Declined")
            <tr>
                <td align="left">{{$prof->type_name or ""}}</td>
                <td align="center">
                    <div id="edit_calender_{{ $prof->holidayrequest_id }}_21" class="edit_cal">
                    <a href="javascript:void(0)" id="date_view_{{ $prof->holidayrequest_id }}_21" />{{ $prof->date }}</a>
                    <span class="glyphicon glyphicon-chevron-down open_adddrop" data-client_id="{{ $prof->holidayrequest_id or "" }}" data-tab="21"></span>
                    <div class="cont_add_to_date open_dropdown_{{ $prof->holidayrequest_id }}_21" style="display:none;">
                    <ul> 
                      <!-- <li><a href="javascript:void(0)" class="open_calender_pop" data-client_id="{{ $prof->holidayrequest_id or "" }}" data-tab="21">Add/Edit Start Date</a></li>
                      <li> -->
                      <span id="view_calender_{{ $prof->holidayrequest_id }}_21" class="addtocalendar atc-style-blue">
                        <var class="atc_event">
                          <var class="atc_date_start">{{ (isset($prof->date) && $prof->date != "")?date("d-m-Y H:i", strtotime($prof->date) ):"" }}</var>
                          <var class="atc_date_end">{{ (isset($prof->date) && $prof->date != "")?date("Y-m-d H:i:s", strtotime('+4 hour', strtotime($prof->date)) ):"" }}</var>
                          <var class="atc_timezone">Europe/London</var>
                          <var class="atc_title">{{$title}} - {{ $prof->type_name or ""}}</var>
                          <var class="atc_description">{{$title}} - {{ $prof->type_name or ""}}</var>
                          <var class="atc_location">Office</var>
                          <var class="atc_organizer">{{ $admin_name }}</var>
                          <var class="atc_organizer_email">{{ $logged_email }}</var>
                        </var>
                ...
                <td align="center">
                    @if(isset($prof->state) && $prof->state == 'Approved')
                        <input type="button" value="{{$prof->state or ""}}" class="approved_btn">
                    @elseif(isset($prof->state) && $prof->state == 'Declined')
               ...
               <td align="center" style="padding: 9px 0; width: 10%;"><button class="btn btn-default note_t">Notes</button></td>
                <td align="center"> 
                    <select class="form-control statechage">
                        ...
                        <!-- <option value="Approved||{{$prof->holidayrequest_id}}" {{ (isset($prof->state) && $prof->state == "Approved")?"selected":"" }}>Approve</option>
                        <option value="Declined||{{$prof->holidayrequest_id}}" {{ (isset($prof->state) && $prof->state =="Declined")?"selected":"" }}>Decline</option> -->
                        <option value="Request Withdrawn||{{$prof->holidayrequest_id}}" {{ (isset($prof->state) && $prof->state == "Request Withdrawn")?"selected":"" }}>Withdraw Request</option>
    	...
    	@if(!empty($awating_staff))
             ...
             @if($staff['archive'] == "archive")
                  ...
                  <tr>
                    ...
                    <a href="javascript:void(0)" class="editrequest" data-rowid="{{$staff['holidayrequest_id']}}">{{$staff['staff_detail']['fname'] or ""}} {{$staff['staff_detail']['lname'] or ""}}</a>
          ...
          <div class="row">  
           <div class="col-xs-12 m_top">          
            <div class="col_m2">
                                ...
                                <div class="j_selectbox">
                                    ...
                                    <div class="open_toggle" style="display: none;">
                                      <ul>
                                        @if(!empty($staff_details))
                                            @foreach($staff_details as $key=>$staff_row)
                                                <li data-value="{{ $staff_row['user_id'] }}">
                                                <div style="float: left; margin-right: 5px;">
                                                <input type="checkbox" class="search_staff_id" name="search_staff[]" value="{{ $staff_row['user_id'] }}"> </div>
                                                <div style="float: left;">
                                                <a href="javascript:void(0)">{{ ucwords(strtolower($staff_row['fname'])) }} {{ ucwords(strtolower($staff_row['lname'])) }}</a>
                                                </div></li>
                                            @endforeach
                                        @endif
                                      </ul>
                ...
                <div class="position_none scroll_content" id="search_data">
                <!-- <table border="1">
                <tr>
                <td valign="top">
                <div class="col-xs-3 position_none" style="width: 310px">
                    <strong>Date</strong>
                    <div class="events_cont">
                        <div class="font_size underline" style="border-bottom: none;"</div>Sunday, February 13, 2016</div>
                    </div>
                    ...
                    @if(!empty($staff_details))
                        @foreach($staff_details as $key=>$staff_row)
                        <!-- <td valign="top">
                            <div class="col-xs-3 position_none" style="width: 290px">
                                <strong>{{ ucwords(strtolower($staff_row['fname'])) }} {{ ucwords(strtolower($staff_row['lname'])) }}</strong>
                                <div class="events_cont">
                                    <ul class="event_ul">
                                        <li>
                                            <div class="ann_leave pull-left"></div> 
                                            <div class="text">Annual leave</div><br>
                                            <div class="font_size"><strong>Time: </strong>Full Day, Half Day-PM</div>
                                        </li>
                                    </ul>
                                    <ul class="event_ul">
                                        <li>
                ...
                <div class="col-xs-12 m_top" style="position: initial !important;">
                    <h5>Calender Key:</h5>
                </div>

                <div class="col-xs-12" style="position: initial !important;">
                    <table>
                        <tr>
     ...
     <!--  <form action="#" method="post"> -->
       ...
       <input type="hidden" name="start_date" value="{{$start_date}}" />
        <div class="modal-body">
                   ...
                   <tr>
                    ...
                    <tr id="staffholi" class="makeCloneClass">
                     ...
                     <td> <input type="text" class="form-control dpick" id="stafdpick" name="date[]" style="width:86%; height: 33px;">
                      ...
                      <td align="center" style="padding: 9px 0; width: 14%;" >
                      <button type="button" class="btn btn-default note_t notesaddfont" id="snotes" data-key="1">Notes</button>
                      
                      <input type="hidden" name="notes[]" id="notesstaff1" value="">
                      </td>
                    </tr>
         ...
         <div class="left_side"><button type="button" class="addnew_line"><i class="add_icon_img"><img src="/img/add_icon.png"></i><p class="add_line_t">Add new</p></button>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td width="12%">
                      ...
                      <td width="15%"><select class="form-control disabled_pop" name="staff" id="editstaff_id">
                    ...
                    <tr id="editstaffholi" class="makeCloneClass">
                     ...
                     <td> <input type="text" class="dpick" id="edittafdpick" name="date" style="width:86%; height: 33px;">
                      ...
                      <td align="center">
                              ...
                              <select class="form-control disabled_pop" id="editdue" name="duration">
                                <option value="Full Day">Full Day</option>
                      ...
                      <td align="center">
                      <select class="form-control disabled_pop" id="edittype" name="requesttype">
                        @if( isset($old_holiday_types) && count($old_holiday_types) >0 )
                            @foreach($old_holiday_types as $key=>$old_row)
                            <option value="{{ $old_row['type_id'] or "" }}" {{ (isset($holiday_details['timeoff_type_id']) && $holiday_details['timeoff_type_id'] == $old_row['type_id'])?'selected':'' }}>{{ $old_row['name'] or "" }}</option>
                            @endforeach
  ...
  <div class="modal-dialog" style="width:36%;">
    ...
    <div class="modal-content">
      ...
      <div style="width:100%;">
        ...
        @endif
        @if($page_open == 1 ) 
        <button class="btn btn-info" onclick="return staffnotes()" id="editsave_staffnotes" style=" text-align: center; margin-top: 15px; float: right;">Save</button>
  ...
  <div class="modal-dialog" style="width:36%;">
    <div class="modal-content">
        ...
        <div style="width:100%;">
          ...
          <textarea rows="4" class="form-control" maxlength="{{ $notesMaxLength }}" name="notes1[]" id="addfontnotesss"></textarea>
         <input type="hidden" id="hidNoteKey" name="hidNoteKey">
         <div class="clr"></div> 
        
        
        
        <button class="btn btn-info" onclick="return staffnotes()" id="save_staffnotes" style="text-align: center; margin-top: 15px; float: right;">Save</button>
...
<div class="modal fade" id="fetchcomposenotes-modal" tabindex="1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" style="width:36%;">
    <div class="modal-content">
      ...
      <div style="width:100%;">
        <label for="f_name" style="font-size: 18px;">Notes</label>
        <textarea rows="4" cols="65"  name="notes1[]" id="fetchnotess" readonly ></textarea>
      ...
      <div class="form-group">
        ...
        <select id="pop_color" name="pop_color" class="form-control">
            ...
            @if( isset($colors) && count($colors) >0 )
                ...
                @if(isset($color_row['color_code']) && $color_row['color_code'] != 'f6ff00' && $color_row['color_code'] != '00c0ef' && $color_row['color_code'] != '000000')
...
<!-- ======================================= -->
<div class="modal fade" id="whoelseis-modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" style="width:990px; ">
    <div class="modal-content">
        ...
        <div class="popup_border">
            <div class="modal-body">
              <table width="100%" border="0" class="show_who_else">
              ...
              <div class="col-xs-12">
                <table>
                    ...
                    <tr>
                        <td colspan="$">
                            <h5><strong>Calender Key:</strong></h5>
                        </td>
                    </tr>
...
<!-- Task Deadline -->
<div class="modal fade" id="task_deadline-modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" style="width:550px; ">
    <div class="modal-content">
        <div class="modal-header">
          ...
          <div class="show_loader"></div>
        <div class="popup_border">
            <div class="modal-body deadScroll">
              <table width="100%" border="0" class="show_deadlines">
...
<!-- Show Tasks All Tab -->
<div class="modal fade" id="show_tasks-modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog" style="width:1250px; ">
  <input type="hidden" name="tasks_staff_id" id="tasks_staff_id">
  <input type="hidden" name="tasks_heading" id="tasks_heading">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close save_btn" data-dismiss="modal" aria-hidden="true">&times;</button>
        </div>
        ...
        <div class="popup_border">
            <div class="modal-body ">
              <table width="100%" border="0" class="table table-bordered table-hover dataTable show_tasks">
...
<!-- Notes Pop Up -->
<div class="modal fade in" id="notes-modal" tabindex="-1" role="dialog" aria-hidden="false">
  <div class="modal-dialog" style="width:500px;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" id="notes_client_id" name="notes_client_id" value="251">
        <input type="hidden" id="notes_tab" name="notes_tab" value="21">
        <table>
          <tbody>
            <tr>
                <td align="left" width="20%"><strong>Notes : </strong></td>
                <td align="left"><textarea cols="56" rows="4" id="notes" name="notes" readonly></textarea></td>
            </tr>
