...
<div class="wrapper row-offcanvas row-offcanvas-left">
    ...
    <div class="tax_top_pan">
      ...
      <span class="show_date">
        <p>
            <a href="javascript:void(0)" class="btn btn-info" style="font-size: 18px; font-weight: bold;">{{ $client_details['initial_badge'] or '' }}</a>
            <a style="margin: 6px 0 0 0;font-size: 18px; font-weight: bold;color:#00acd6">{{ $client_details['client_name'] or '' }}</a>
        </p>
      </span>
...
<div class="tabarea">
  ...
  <div class="nav-tabs-custom">
      <ul class="nav nav-tabs nav-tabsbg" id="header_ul">
        ...
        <li class="{{ ($page_open == 2 || $page_open == 21 || $page_open == 22)?'active':'' }}"><a href="javascript:void(0)" data-open_id="2" class="" id="non_clickable">TAX RETURN INFORMATION</a></li>
...
{{ Form::open(array('url'=>'/individual/insert-client-details', 'files' => true, 'id'=>'basicform')) }}
    ...
    <input type="hidden" id="initial_badge" value="{{ $client_details['initial_badge'] or '' }}">
...
<div id="step2" class="tab-pane {{ ($page_open == 2 || $page_open == 21 || $page_open == 22)?'active':'' }}">
  <div class="box-body table-responsive">
    <div role="grid" class="dataTables_wrapper form-inline" id="example2_wrapper">
      <div class="row">
        <div class="col-xs-6"></div>
                ...
                <td width="30%">
                 ...
                 <div class="j_selectbox002">
                  ...
                  <div class="open_toggle" style="z-index: 99;">
                    <ul id="document_list">
                    @if(isset($checklist_docs['documents']) && count($checklist_docs['documents']) >0)
                      @foreach($checklist_docs['documents'] as $key=>$value)
...
<div class="col_m2">  
      <div class="nav-tabs-custom">
            <ul class="nav nav-tabs nav-tabsbg" id="header_ul">
              <li class="{{ ($page_open == 2 || $page_open == 21)?'active':'' }}"><a href="javascript:void(0)" data-open_id="21">MESSAGES</a></li>
              <li class="{{ ($page_open == 22)?'active':'' }}"><a href="javascript:void(0)" data-open_id="22">DOCUMENTS UPLOAD</a></li>
            </ul>
        <div class="tab-content">

     <div id="step21" class="tab-pane {{ ($page_open == 2 || $page_open == 21)?'active':'' }}">
         <input type="hidden" id="hidd_reply_id" value="0">
        <div class="form-group" id="addnew_box">
          <div class="col-xs-2 no-pad">
            <button type="button" class="btn btn-info" id="open_msg_box">Add New Note</button>
          </div>
       ...
       <div class="form-group" id="blankMsgDiv">
         <div class="col-xs-12">
             <div id="msg_box" class="col-xs-8 no-pad" style="display: none">   
              <div class="form-group">
                  <div class="input_icon blank_box">
              ...
              <div class="form-group">
                <div class="input_icon" style="margin-left: 2px;"></div>
                <div class="col-xs-10 no-pad">
                  <textarea name="" placeholder="Write a message.." id="message0" rows="3" class="form-control classy-editor"></textarea>
                  <button class="btn btn-primary input_textbox save_message" style="float: right;" type="button" data-reply_id="0">Send Message</button><!--save_message-->
                </div>
                <div class="col-xs-1 msg_loader" id="loader0"></div>
                <div class="clearfix"></div>
              </div>
               <div class="clearfix"></div>
          </div>   
           ...
           <div class="clearfix"></div>
            ...
            @if(isset($checklist['messages']) && count($checklist['messages']) >0)
                @foreach($checklist['messages'] as $key=>$message)   
                  <div class="form-group messagePrepend">
                  ...
                  @if(isset($message['messages']) && count($message['messages']) >0)
                    ...
                    <div id="msg_div{{ $message['message_id'] or '' }}">
                        <div class="input_icon" style="height: 50px;">
                            <a href="javascript:void(0)" class="btn btn-info" style="font-size: 18px; font-weight: bold;">
                                {{ $row['from_bladge'] or '' }}
                            </a>
                        </div>
                        <div class="col-xs-10 no-pad">
                            ...
                            <div class="col-xs-2">
                              <p>{{ (isset($row['created']) && $row['created'] != '0000-00-00 00:00:00')?date('M d, Y', strtotime($row['created'])):'' }}</p>
                            </div>
                        </div>
                        <div class="clearfix"></div>
                    </div>
                ...
                @endif
                  ...
                  <div class="form-group">
                    ...
                    <div class="input_icon blank_box"></div>
                    <div class="col-xs-10 no-pad">
                        <textarea placeholder="Write a message.." id="message{{ $message['message_id'] or '' }}" rows="3" class="form-control classy-editor"></textarea>
                      <button class="btn btn-primary input_textbox save_message" style="float: right;" type="button" data-reply_id="{{ $message['message_id'] or '' }}">Send Message</button><!--save_message-->
                    </div>
                    <div class="col-xs-1 msg_loader" id="loader{{ $message['message_id'] or '' }}"></div>
                    </div>
                    <div class="clearfix"></div>
                  </div> 
          ...
          <div id="step22" class="tab-pane {{ ($page_open == 22)?'active':'' }}">
              <div class="col-xs-12" style="margin-top: 20px;">
                  <div class="col-xs-8">
       
                    <div style="margin:0 auto; width:60%;">
                    <form method="POST" action="/pdfclientupload" accept-charset="UTF-8" id="clientpdfeform14" name="clientpdfeform14" enctype="multipart/form-data"><input name="_token" type="hidden" value="A9bH09IOnOoJBSzBBxNAnNXYAq4g6rgaL2nM313b">
                        <span class="btn btn-default btn-file" style="float:left; margin-right: 10px; width: 80px;"> Upload
                      ...
                      <span class="pull-right">
                        <div class="j_selectbox2" style="width:300px !important;">
                           ...
                           <div class="open_toggle" style="display: none;">
                             <ul class="doc_list">
                                 @if(isset($client_docs) && count($client_docs)>0)
                                    @foreach($client_docs as $key=>$value)
                                      <li data-value="non">
                                          <a href="javascript:void(0)" data-client="{{ $value['client_id'] or '0' }}" data-file="{{ $value['file'] or '' }}" id="{{ $value['clienttaxpdf_id'] or '0' }}" class="filename">{{ $value['file'] or '' }}</a>
                                          <strong style="float: right;"><a href="javascript:void(0)" class="deldoc" data-delid="delid_{{ $value['clienttaxpdf_id'] or '0' }}">
                                          <img src="/img/cross.png"></a></strong>
                                      </li>
                                    @endforeach
                                  @endif
                             </ul>
